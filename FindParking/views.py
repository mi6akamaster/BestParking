...
def FindParking(request):  
    ...
    if request.method == 'GET':
        ...
        context = {'addressForm':address_from,'subscribeForm':subscribe_form,'loadDefaultParkings':"defaultSofia",'parkings':parkings, 'features':features, 'paymentMethods':payment_methods,
        ...
        if 'subsribeForm' in request.POST:
            try:
                ...
                if subscribe_form.is_valid():
                    ...
                    if is_parkingowner == True:
                        ...
                        to_add.save()
                        #send email
                    context = {'message':"Благодарим Ви!",'openSubscribe':"openAndRefresh", 'addressForm':address_from,'subscribeForm':subscribe_form, 'domain':Site.objects.get_current().domain}
                    return render_to_response('findparking.html', context, context_instance=RequestContext(request))
                context = {'message':"Въведете валиден адрес!",'openSubscribe':"shouldOpen",'addressForm':address_from,'subscribeForm':subscribe_form, 'domain':Site.objects.get_current().domain}
                return render_to_response('findparking.html', context, context_instance=RequestContext(request))
            except IntegrityError:
                context = {'message':"Адресът вече е въведен!",'openSubscribe':"shouldOpen",'addressForm':address_from,'subscribeForm':subscribe_form, 'domain':Site.objects.get_current().domain}
                return render_to_response('findparking.html', context, context_instance=RequestContext(request))
            ...
            if address_from.is_valid():
                ...
                return render_to_response('findparking.html', {'addressForm':address_from,'subscribeForm':subscribe_form,
...
def ajaxCall(request, latlng):
    ...
    features = [ParkingFeatures.objects.get(id=parking.features_id) for parking in ParkingMarker.objects.all() if distance([parking.lat, parking.lng], [float(lat), float(lng)]) < 0.5]
    ...
    payment_methods = [PaymentMethod.objects.get(id=parking.paymentMethod_id) for parking in ParkingMarker.objects.all() if distance([parking.lat, parking.lng], [float(lat), float(lng)]) < 0.5];
